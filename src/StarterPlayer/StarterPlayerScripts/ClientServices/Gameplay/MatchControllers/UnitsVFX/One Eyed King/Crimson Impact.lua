...
function CrimsonImpact:Activate(Unit,UnitInfo)
    warn("sup cuh")
    local HumanoidRootPart = Unit.RootPart
    local Impact = UnitAssets.Kaneki.Impact:Clone()
    local Track = Unit.AnimationController:LoadAnimation(Animations.CrimsonImpact)

    Track:Play()

    Impact.Parent = Unit
    Impact:PivotTo(HumanoidRootPart.CFrame * CFrame.new(0.459469795, -1.23084259, -5.13952637, 1, 0, 0, 0, 1, 0, 0, 0, 1))-- * CFrame.new(-0.107395172, 0.026473999, -0.901611328, -1, 0, 0, 0, -1, 0, 0, 0, 1)
...
function CrimsonImpact.Attack(Unit,UnitInfo)
    UnitService = Knit.GetService("UnitService")
    CrimsonImpact:Activate(Unit,UnitInfo)
end

return CrimsonImpact