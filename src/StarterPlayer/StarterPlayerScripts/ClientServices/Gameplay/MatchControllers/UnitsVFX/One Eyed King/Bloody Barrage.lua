...
local UnitAssets = VFX.OneEyedKing
local Animations = ReplicatedStorage.Assets.Animations.OneEyedKing
...
function BloodyBarrage:Activate(Unit,UnitInfo)
    local HumanoidRootPart = Unit.RootPart
    local Barrage = UnitAssets.barrage:Clone()
    local Track = Unit.AnimationController:LoadAnimation(Animations.BloodyBarrage)

    Track:Play()

    Barrage.Parent = workspace
    Barrage:PivotTo(HumanoidRootPart.CFrame * CFrame.new(-0.107395172, 0.026473999, -0.901611328, -1, 0, 0, 0, -1, 0, 0, 0, 1))
    ...
    Track:GetMarkerReachedSignal("End"):Connect(function()
        Enable(Barrage, false)

        Debris:AddItem(Barrage,1.5)
    end)
end

function BloodyBarrage.Attack(Unit,UnitInfo)
    UnitService = Knit.GetService("UnitService")
    BloodyBarrage:Activate(Unit,UnitInfo)
end

return BloodyBarrage