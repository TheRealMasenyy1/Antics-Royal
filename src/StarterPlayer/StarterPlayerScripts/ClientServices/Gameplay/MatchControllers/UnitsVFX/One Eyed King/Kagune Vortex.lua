...
function KaguneVortex:Activate(Unit,UnitInfo)
    warn("sup cuh")
    local HumanoidRootPart = Unit.RootPart
    local Spin = UnitAssets.Kaneki.Spin:Clone()
    local Track = Unit.AnimationController:LoadAnimation(Animations.KaguneVortex)

    Track:Play()

    Spin.Parent = Unit
    Spin:PivotTo(HumanoidRootPart.CFrame)-- * CFrame.new(-0.107395172, 0.026473999, -0.901611328, -1, 0, 0, 0, -1, 0, 0, 0, 1)
...
function KaguneVortex.Attack(Unit,UnitInfo)
    UnitService = Knit.GetService("UnitService")
    KaguneVortex:Activate(Unit,UnitInfo)
end
