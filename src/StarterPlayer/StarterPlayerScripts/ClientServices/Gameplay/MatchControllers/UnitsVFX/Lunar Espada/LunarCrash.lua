...
function LunarCrash:Activate(Unit,UnitInfo)
    local HumanoidRootPart = Unit.HumanoidRootPart
    local LunarCrashImpact = UnitAssets.LunarCrash

    local clonedUnit = Unit:Clone()

    LunarCrashImpact.Parent = workspace.Debris

    LunarCrashImpact:PivotTo(UnitInfo.Target.CFrame)
    ...
    Debris:AddItem(LunarCrashImpact,1)
...
function LunarCrash.Attack(Unit,UnitInfo)
    UnitService = Knit.GetService("UnitService")
    LunarCrash:Activate(Unit,UnitInfo)
end
