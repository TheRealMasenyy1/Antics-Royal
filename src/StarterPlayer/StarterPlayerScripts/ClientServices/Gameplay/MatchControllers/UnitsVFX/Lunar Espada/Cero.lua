...
function Cero:Activate(Unit,UnitInfo)
    local HumanoidRootPart = Unit.HumanoidRootPart
    local Cero = UnitAssets.Cero:Clone()

    Cero.CFrame = HumanoidRootPart.CFrame * CFrame.Angles(0,math.rad(90),0)
    Cero.Parent = Unit
...
function Cero.Attack(Unit,UnitInfo)
    UnitService = Knit.GetService("UnitService")
    Cero:Activate(Unit,UnitInfo)
end
