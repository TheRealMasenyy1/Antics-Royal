...
function EspadaSlash:Activate(Unit,UnitInfo)
    local HumanoidRootPart = Unit.HumanoidRootPart

    local attachments = SetAttachments(UnitAssets.Attachments,Unit)
...
function EspadaSlash.Attack(Unit,UnitInfo)
    UnitService = Knit.GetService("UnitService")
    EspadaSlash:Activate(Unit,UnitInfo)
end
