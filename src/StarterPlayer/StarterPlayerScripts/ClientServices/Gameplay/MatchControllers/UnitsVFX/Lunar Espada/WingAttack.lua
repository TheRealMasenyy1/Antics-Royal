...
local UnitAssets = VFX.LunarEspada
...
function WingAttack:Activate(Unit,UnitInfo)
    local HumanoidRootPart = Unit.HumanoidRootPart
    local WingAttackImpact = UnitAssets.WingAttack

    local clonedUnit = Unit:Clone()

    WingAttackImpact.Parent = workspace.Debris

    WingAttackImpact:PivotTo(UnitInfo.Target.CFrame)

    clonedUnit.Parent = workspace.Debris
    clonedUnit:PivotTo(Unit.HumanoidRootPart.CFrame)

    local runTween = TweenService:Create(clonedUnit.HumanoidRootPart,TweenInfo.new((45/60) * 1),{CFrame = UnitInfo.Target.CFrame})
    ...
    Invisible(clonedUnit,0)

    EmitAll(WingAttackImpact)
    ...
    Debris:AddItem(clonedUnit,1)
    Debris:AddItem(WingAttackImpact,1)
...
function WingAttack.Attack(Unit,UnitInfo)
    UnitService = Knit.GetService("UnitService")
    WingAttack:Activate(Unit,UnitInfo)
end
