...
function ThunderboltStrike:Activate(Unit,UnitInfo)
    local HumanoidRootPart = Unit.HumanoidRootPart
    local Lightning = UnitAssets.Lightning:Clone()

    Lightning:PivotTo(UnitInfo.Target.CFrame)
    Lightning.Parent = Unit

    Lightning.Lightning.Smoke.Smoke.Enabled = true
    Lightning.Lightning.First.Lightning.Enabled = true

    task.wait(.3)

    Emit(Lightning.Lightning.Smoke.Attachment)

    task.wait(.05)

    Emit(Lightning.Lightning)
    UnitService:RequestDamage(Unit)

    task.wait(.55)

    Lightning.Lightning.Smoke.Smoke.Enabled = false
    Lightning.Lightning.First.Lightning.Enabled = false
end

function ThunderboltStrike.Attack(Unit,UnitInfo)
    UnitService = Knit.GetService("UnitService")
    ThunderboltStrike:Activate(Unit,UnitInfo)
end
