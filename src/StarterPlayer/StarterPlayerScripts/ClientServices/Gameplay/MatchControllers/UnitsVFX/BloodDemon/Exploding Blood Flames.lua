...
local ExplodingBloodFlames = {}

local VFX = ReplicatedStorage.Assets.VFX
local UnitAssets = VFX.BloodDemon
local Animations = ReplicatedStorage.Assets.Animations.BloodDemon
...
function ExplodingBloodFlames:Activate(Unit,UnitInfo)
    local HumanoidRootPart = Unit.HumanoidRootPart
    local ExplodingFlames = UnitAssets["Exploding Blood Flames"]:Clone()
    local Track = Unit.Humanoid.Animator:LoadAnimation(Animations.BloodBurst)

    Track:Play()

    ExplodingFlames.Parent = Unit


    Track:GetMarkerReachedSignal("Hit"):Connect(function()
        ExplodingFlames.CFrame = UnitInfo.Target.CFrame

        Emit(ExplodingFlames.A0)
        
        UnitService:RequestDamage(Unit)

        Debris:AddItem(ExplodingFlames,1.5)
    end)
    
end

function ExplodingBloodFlames.Attack(Unit,UnitInfo)
    UnitService = Knit.GetService("UnitService")
    ExplodingBloodFlames:Activate(Unit,UnitInfo)
end

return ExplodingBloodFlames