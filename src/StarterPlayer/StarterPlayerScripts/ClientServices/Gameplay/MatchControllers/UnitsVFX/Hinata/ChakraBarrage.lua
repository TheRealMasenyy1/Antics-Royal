...
function ChakraBarrage:Activate(Unit,UnitInfo)
    local HumanoidRootPart = Unit.HumanoidRootPart
    local Barrage = UnitAssets["Gentle Fist Barrage"]:Clone()
    local Track = Unit.Humanoid.Animator:LoadAnimation(Animations.GentleFist)

    Track:Play()

    Barrage.Parent = Unit
    Barrage.CFrame = HumanoidRootPart.CFrame * CFrame.new(-0.107395172, 0.026473999, -0.901611328, -1, 0, 0, 0, -1, 0, 0, 0, 1)
...
function ChakraBarrage.Attack(Unit,UnitInfo)
    UnitService = Knit.GetService("UnitService")
    ChakraBarrage:Activate(Unit,UnitInfo)
end

return ChakraBarrage