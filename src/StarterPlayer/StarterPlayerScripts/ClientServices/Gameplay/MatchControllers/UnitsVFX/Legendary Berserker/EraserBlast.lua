...
function EraserBlast:Activate(Unit,UnitInfo)
    local HumanoidRootPart = Unit.HumanoidRootPart
    local EraserBlast = UnitAssets.EraserBlast:Clone()
    local Explosion = UnitAssets.Explosion:Clone()
    local Track : AnimationTrack = Unit.Humanoid.Animator:LoadAnimation(Animations.Ball)
    local SoundController = Knit.GetController("SoundController")

    local posB = HumanoidRootPart.CFrame * CFrame.new(math.random(6,6),math.random(3,4),-10)
    local growing = true

    Track:Play() -- BrolyKiBlast

    Track:GetMarkerReachedSignal("Charge"):Connect(function()
        Track:AdjustSpeed(.8)
        Enable(EraserBlast,true)
        EraserBlast.Parent = Unit
        local currentScale = .01
        while growing do
            task.wait(1/120)
            currentScale += 0.005
            EraserBlast:PivotTo(Unit["Left Arm"].LeftGripAttachment.WorldCFrame)

            EraserBlast:ScaleTo(currentScale)

           --for _,part in ipairs(EraserBlast:GetChildren()) do
    ...
    Track:GetMarkerReachedSignal("Fire"):Connect(function()
        ...
        SoundController:Play("BrolyKiBlast",{Parent = HumanoidRootPart})

        for i = 0, 1, 0.023 do
            EraserBlast:PivotTo(CFrame.new(quadBezier(i, HumanoidRootPart.Position, posB.Position, UnitInfo.Target.Position)))
            task.wait()
        end
    
        Explosion.CFrame = UnitInfo.Target.CFrame
        Explosion.Parent = Unit
    
        Emit(Explosion.Attachment)
    
        TweenService:Create(Explosion.Attachment.Light,TweenInfo.new(1),{Range = 0}):Play()
...
function EraserBlast.Attack(Unit,UnitInfo)
    UnitService = Knit.GetService("UnitService")
    EraserBlast:Activate(Unit,UnitInfo)
end
