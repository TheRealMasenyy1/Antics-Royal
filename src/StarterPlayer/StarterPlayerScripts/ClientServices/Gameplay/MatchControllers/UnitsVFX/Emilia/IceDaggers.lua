...
local UnitAssets = VFX.Emilia
local Animations = ReplicatedStorage.Assets.Animations.Emilia
...
function IceDaggers:Activate(Unit,UnitInfo)
    local HumanoidRootPart = Unit.HumanoidRootPart
    local IceDaggersBundle = UnitAssets.IceDaggers:Clone()
    local Track = Unit.Humanoid.Animator:LoadAnimation(Animations.IceRain)

    local oldSize

    oldSize = IceDaggersBundle.Dagger.Size

    print("Naur")

    IceDaggersBundle:PivotTo((HumanoidRootPart.CFrame * CFrame.new(0,5,0)) * CFrame.Angles(math.rad(180),0,0))
    IceDaggersBundle.Parent = workspace

    task.wait(2)

    for _,dagger in pairs(IceDaggersBundle:GetChildren()) do
        dagger.Size = Vector3.new(0.1,0.1,0.1)
    ...
    local thread = task.spawn(function()
        while true do
            task.wait()
            for _,dagger in pairs(IceDaggersBundle:GetChildren()) do
                dagger.CFrame = CFrame.new(dagger.CFrame.Position,UnitInfo.Target.CFrame.Position + Vector3.new(0,3,0)) * CFrame.Angles(math.rad(180),math.rad(20),0)
            end
        end
    end)

    for _,dagger in pairs(IceDaggersBundle:GetChildren()) do
        TweenService:Create(dagger,TweenInfo.new(.75),{Size = oldSize, Transparency = .15}):Play()
    end
    ...
    Track:GetMarkerReachedSignal("Hit"):Connect(function()
        ...
        for _,dagger in pairs(IceDaggersBundle:GetChildren()) do
            TweenService:Create(dagger,TweenInfo.new(.15),{Position = UnitInfo.Target.CFrame.Position}):Play()
    
            task.wait(.05)
    
            task.delay(1,function()
                TweenService:Create(dagger,TweenInfo.new(.45),{Transparency = 1}):Play()
            end)
    
            Debris:AddItem(dagger,2)
        end
    
        IceDaggersBundle:Destroy()
    end)


end

function IceDaggers.Attack(Unit,UnitInfo)
    UnitService = Knit.GetService("UnitService")
    IceDaggers:Activate(Unit,UnitInfo)
end
