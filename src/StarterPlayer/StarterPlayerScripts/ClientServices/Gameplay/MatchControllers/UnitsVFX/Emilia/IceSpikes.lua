...
function IceSpikes:Activate(Unit,UnitInfo)
    local HumanoidRootPart = Unit.HumanoidRootPart
    local IceSpikes = UnitAssets.IceSpikes:Clone()
    local Track = Unit.Humanoid.Animator:LoadAnimation(Animations.IceSpikes)
    ...
    Track:GetMarkerReachedSignal("Hit"):Connect(function()
        ...
        for _,Spike in pairs(IceSpikes:GetChildren()) do
            sizeTable[Spike.Name] = Spike.Size
    
            Spike.Size = Vector3.new(Spike.Size.X,0.01,Spike.Size.Z)
        end
    
        IceSpikes:PivotTo(CFrame.new(UnitInfo.Target.CFrame.Position,Unit.PrimaryPart.Position) * CFrame.new(0,-2,0))
    
        IceSpikes.Parent = Unit
        
        for _,Spike in pairs(IceSpikes:GetChildren()) do
            TweenService:Create(Spike,TweenInfo.new(.4),{Size = sizeTable[Spike.Name]}):Play()
    
            task.delay(1.5,function()
                TweenService:Create(Spike,TweenInfo.new(.3),{Transparency = 1}):Play()
            end)
        end
    
        UnitService:RequestDamage(Unit)
    
        Debris:AddItem(IceSpikes,3)
    end)
end

function IceSpikes.Attack(Unit,UnitInfo)
    UnitService = Knit.GetService("UnitService")
    IceSpikes:Activate(Unit,UnitInfo)
end
