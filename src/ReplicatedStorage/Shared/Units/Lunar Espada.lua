...
local LunarEspada = {}
LunarEspada.__index = setmetatable(LunarEspada,UnitManager)
...
function LunarEspada.Setup(Unit, CustomUnitInfo) 
	local self = UnitManager.new(Unit,CustomUnitInfo)
	setmetatable(self,LunarEspada)
	...
	self.Type = "Ground"
	self.Ability = "Cero"--"EspadaSlash"--"Cero"--"BreakerRay"
...
function LunarEspada:Attack(Enemy)
