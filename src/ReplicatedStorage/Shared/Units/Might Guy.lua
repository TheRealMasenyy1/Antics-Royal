...
local MightGuy = {}
MightGuy.__index = setmetatable(MightGuy,UnitManager)
...
function MightGuy.Setup(Unit, CustomUnitInfo) 
	local self = UnitManager.new(Unit,CustomUnitInfo)
	setmetatable(self,MightGuy)
	...
	self.Type = "Ground"
	self.Ability = "Power Kick"--"Power Punch"--"IceDaggers"
...
function MightGuy:Attack(Enemy)
